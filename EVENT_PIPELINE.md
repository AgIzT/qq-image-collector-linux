...
## 2026-08-04 PNG 大型元数据与单图容错

生产恢复页出现了超过 Pillow 默认 1 MiB 压缩文本限制的 PNG 工作流。解析器现允许
单个 PNG 文本块解压后最多 16 MiB、整图文本最多 64 MiB；手工读取 `zTXt` 和压缩
`iTXt` 时使用同一 16 MiB 上限的流式有界解压，禁止压缩炸弹无界占用内存。这个上限
只扩大真实 ComfyUI/生成参数的兼容范围，不改变 128 MiB 单文件下载边界。

CDN 文件完整落盘后先计算 SHA-256，再执行元数据解析。Pillow、zlib 对损坏图片、损坏
压缩流、异常尺寸或超限文本抛出的已知解码异常只终止当前图片：记录为
`rejected_no_metadata`，保存 SHA-256 和不含异常正文的错误类型，删除临时文件，然后
继续领取下一张。单张恶意或损坏图片不得重启 Worker，也不得永久卡住逐页排空恢复。

生产历史页还会把商城表情以 `gxh.vip.qq.com/.../raw300.gif` 放进普通图片段；该精确
腾讯主机现加入 HTTPS 白名单。普通资源仍必须经过流开头 `GIF87a/GIF89a` 魔数确认。
唯一例外是事件同时给出表情信号，HTTPS 主机为 `gxh.vip.qq.com`/`p.qpic.cn`，且路径
严格匹配 `/club/item/parcel/item/.../raw300.gif`：生产已观测到部分下架资源重定向至
`p.qpic.cn` 后返回 404，永远无法再取得魔数，因此三项证据同时满足时直接进入
`filtered_gif`。不能仅凭后缀淘汰，也不能把已下架表情伪装成“短 ID 失效”阻塞页面。
任意其他主机或路径继续失败关闭。
...
从 JSON 回填一次。尚未过期的 URL 按 `url_expires_at` 最早到期优先，已过期或没有
期限提示的任务再按 `discovered_at` FIFO；两级查询均使用部分索引，不再在每张图片的
热路径解析或创建临时排序树。HTTP 400/403/404/410 刷新得到不同 URL 后立即在同一任务中
重试，不再把它重新排到整条队列末尾。生产默认图片间隔为 0，单并发下载完成一张后
立即领取下一张；只有真实网络耗时和针对当前失败项的重试退避会等待。窗口恢复不再
固定等待 60 秒，每页图片与计数在同一事务提交，避免为每张历史图片反复争抢写锁；
进度快照与日志最多每 2 秒发布一次，但历史页请求本身不等待。

控制台“持久队列”状态只表示 SQLite 队列是否可读；积压数量和最老年龄显示为
“处理中”，不能再把超过 2 小时的正常积压标成“未就绪”。停采由 Worker、事件流和
下载器心跳独立报警。

生产复核随后发现两项会让旧积压空转的问题。第一，早期 6 小时期望值的迁移只匹配
`url_expires_at == discovered_at + 21600`，URL 捕获与入库相差一秒时不会命中，导致
数千条已失效 URL 仍被当作“新鲜”；迁移现按旧 `url_expiry_basis` 一次性换算 JSON
和结构化列。第二，窗口历史响应的短 `message_id` 只存在于 NapCat 内存映射，翻过
数千页后再逐图刷新会返回“消息不存在”。恢复器因此启用持久化逐页排空：每页 URL
入队后必须等该页图片进入终态才翻下一页；20 分钟仍未排空则用该页原锚点重放，不能
再次把整段历史 URL 一次性灌入队列。每条恢复图片同时保存当页短 ID 作为即时刷新锚点，
但稳定去重身份仍是 `real_seq`。
常驻 Worker 若发现固定恢复窗口内只有已失效短 ID 的旧行，会将该行延后并明确标记
`awaiting bounded page replay`，不再逐条调用已知必失败的历史接口；恢复器重放到该页时
会用新 URL 自动解除延后。实时 raw `msgId` 的普通断档恢复不受此规则影响。
...
作为淘汰条件。2026-08-03 生产侧不读正文的普通 GET 探针观测到：带 rkey 的
`multimedia.nt.qq.com.cn` 样本在约 30 分钟仍返回 200，约 60 分钟已返回 400；因此
`url_expires_at` 使用保守 30 分钟调度窗口。它不是精确 TTL 声明。未过期任务按最早
期限处理，过期任务保持 FIFO 并刷新；无 rkey URL 不伪造期限。

候选 URL 按配置顺序逐个做 HTTPS/主机白名单验证。首选非法、400、403、404 或 410
时先尝试事件已有的第二个合法候选；两条均失效后用该消息的 raw `msgId` 刷新 URL，
取得不同 URL 后立即重试当前图片。
一旦结构化期限已过，带 rkey 的候选不会再发出注定失败的 CDN 请求：同图存在无 rkey
候选时直接使用，否则先刷新消息 URL。无期限提示的 URL 仍实际请求，不能凭年龄臆断
失效。
...
6. 每群每页之间没有人为等待；每页串行请求并批量提交 SQLite。没有每小时、每日、
   每群页数或队列深度停止条件；
